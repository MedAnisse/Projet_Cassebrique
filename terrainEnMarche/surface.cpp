...
bool surface::collision(cassebrique::balle* balle)
{
   if((distance(balle) <= 0 ))
        {
            balle->rebond();
...
double surface::distance(balle* b )
{
    ...
    }else
    {
        if(d_debut.y()==d_fin.y())
            {
                geom::point balleCentre=b->position();
                if(balleCentre.x()>=d_debut.x()&&balleCentre.x()<=d_fin.x()||balleCentre.y()<=d_debut.y()&&balleCentre.y()>=d_fin.y())
                {geom::point j{balleCentre.x(),d_debut.y()};// oj segment perpendiculaire sur la surface ab, y de o égal a y de j si la surface est vertical
                return balleCentre.distance(j)-b->rayon();}
            }
    }
return 1.0;
}
